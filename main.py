...
def generate_summary(text):
    prompt = f"""
以下は掲示板の書き込み内容です。
この内容から、目を引く長い日本語タイトルをひとつ考えてください。
出力ルール
タイトルのみを出力してください（前置き・後書き禁止）
「タイトルを提案します」「以下のタイトルです」などの語は書かないでください
タイトル文を 「」で囲まないでください
120文字以内に収めてください
禁止事項
ネガティブ表現や差別・中傷・煽動的な語句を一切使用しないこと
使用禁止例：意味不明、共産主義、中国人、血税、糞尿、悩む、スケベ、低俗、トラブル、酷い、劣等感 など
特定の国籍・民族・宗教・属性を直接名指しする表現を避けること
暴力・猥褻・差別・誹謗中傷を示唆・助長する内容を含めないこと
取り上げる話題（いずれか）
