...
### 手順
1. {text} を読み、次の「許可カテゴリ」に該当する話題が含まれるか判定し、存在しなければ **"NOK"** とだけ答える。  
2. 「禁止語リスト」にある語句・表現がタイトルに含まれないよう注意しながら、目を引く長い日本語タイトルを1つ作成する。  
3. 出力は **タイトルのみ**。120文字以内。前置き・後書き禁止。括弧で囲まない。  
4. ハッシュタグ・URLはあってもなくても可（本文にあれば維持してよい）。
### 許可カテゴリ
物件概要, 価格・コスト, 交通, 構造・建物, 共用施設, 設備・仕様, 間取り, 買物・食事, 育児・教育, 環境・治安, 周辺施設
### 禁止語リスト（すべてNG）
意味不明, 共産主義, 中国人, 血税, 糞尿, 悩む, スケベ, 低俗, トラブル, 酷い, 劣等感
（※同義・近義語や差別・中傷語、国籍や属性を名指しする表現も含め禁止）
